...
const getSingleMyRentalOrdersFromDB = async (
  customerId: string,
  orderId: string,
) => {
...
const updateMyRentalOrderStatusFromDB = async (
  customerId: string,
  orderId: string,
  payload: RentalOrderStatus,
) => {
  ...
  if (payload !== "PENDING") {
    throw new Error(
      "Customer can only update the order status when it is PENDING",
    );
  }
...
const updateProvidersRentalOrderStatusFromDB = async (
  providerId: string,
  orderId: string,
  payload: RentalOrderStatus,
) => {
  ...
  if (payload === "PICKED_UP" && currentStatus !== "PAID") {
    throw new Error(
      "Order can only be marked as PICKED_UP after the customer has PAID",
    );
  }
  if (payload === "RETURNED" && currentStatus !== "PICKED_UP") {
    throw new Error(
      "Order can only be marked as RETURNED after the gear has been PICKED_UP",
    );
  }

  if (payload === "RETURNED" || payload === "REJECTED") {
